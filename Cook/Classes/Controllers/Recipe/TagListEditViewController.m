...
- (id)initWithEditView:(UIView *)editView delegate:(id<CKEditViewControllerDelegate>)delegate selectedItems:(NSArray *)selectedItems editingHelper:(CKEditingViewHelper *)editingHelper
{
    ...
    if (self) {
        ...
        self.items = [[CKBookManager sharedInstance].tagArray sortedArrayUsingComparator:^NSComparisonResult(CKRecipeTag *obj1, CKRecipeTag *obj2) {
...
- (void)targetTextEditingViewDidAppear:(BOOL)appear {
    ...
    helpLabel.text = [NSLocalizedString(@"Tag your recipe appropriately to help other people find it", nil) uppercaseString];
