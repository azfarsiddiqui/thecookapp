...
-(void)setMaskImage:(UIImage *)maskImage atCenter:(CGPoint)center {
    [self setMaskImage:maskImage atCenter:center offset:UIOffsetZero];
}

-(void)setMaskImage:(UIImage *)maskImage atCenter:(CGPoint)center offset:(UIOffset)offset {
	CGPoint position = CGPointMake((int)(center.x - maskImage.size.width/2.0) + offset.horizontal, (int)(center.y - maskImage.size.height/2.0) + offset.vertical);
...
{
    [self setMaskWithText:text font:font offset:UIOffsetZero];
}

-(void)setMaskWithText:(NSString *)text font:(UIFont *)font offset:(UIOffset)offset
{
	...
	[self setMaskImage:maskImage atCenter:center offset:offset];
}
